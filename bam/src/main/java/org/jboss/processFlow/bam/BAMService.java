...
@RemoteBinding(jndiBinding="BAMService/remote")

@Service (objectName="org.jboss.processFlow.bam:service=BAMService")
@Management(IBAMService.class)
@Depends ({
    "org.jboss.processFlow.bam:application=bamEMF,type=statistics"
})
public class BAMService implements IBAMService, MessageListener {
    ...
    private static EntityManagerFactory jbpmBamEFactory = null;
    ...
    public void start() throws Exception {
        ...
        try {
            ...
            jbpmBamEFactory = (EntityManagerFactory)jndiContext.lookup("java:/jbpm-bam-EMF");
    ...
    public List<ProcessInstanceLog> getActiveProcessInstanceLogsByProcessId(String processId) {
        return JPAProcessInstanceDbLog.findActiveProcessInstances(processId);
           ...
           /*  Moved from similar method in KnowledgeSessionService .... not sure whether to use this query or query from JPAProcessInstanceDbLog
             return em.createQuery(
                    "select new org.jbpm.process.audit.ProcessInstanceLog (p.processInstanceId, p.processId)"
    ...
    public void onMessage(Message message) {
        ...
        try {
            //log.info("onMessage() trnx status = "+tMgr.getStatus()+" : trnx = "+tMgr.getTransaction());
            if(message instanceof BytesMessage) {
                ...
                if (logEventType == AsyncBAMProducer.AFTER_SUBPROCESSINSTANCE_CREATED
                    ...
                    // to relect the changes on audit trail
                    flushBam();
                    ...
                    // Nick: the end date should be passed by producer, to eliminate delay of the network transporting
    ...
    public void stop() throws Exception {
        ...
        if(connectObj != null)
            connectObj.close();
        if(persistManager.isOpen()) {
            // no need for a new trnx .... service pojo already initiates within a trnx
            persistManager.joinTransaction();
            persistManager.flush();
            persistManager.clear();
        }
